...
class TBase(TLocation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.facilities = []  # List of TFacility
        self.units = []       # List of units in base
        self.items = {}       # Dict of item_id -> quantity
        self.captures = []    # List of captured units (prisoners)
        self.crafts = []      # List of crafts in hangar/garage
    ...
    def can_build_facility(self, facility_type: TFacilityType):
        # Check max per base
        count = sum(1 for f in self.facilities if f.type.id == facility_type.id)
        if count >= facility_type.max_per_base:
            return False
        # Check required facilities
        for req in facility_type.facility_needed:
            if not any(f.type.id == req and f.is_active() for f in self.facilities):
                return False
    ...
    def get_active_facilities(self):
        return [f for f in self.facilities if f.is_active()]
