"""
XCOM Lore Module: calendar.py

Manages the in-game calendar, campaign scheduling, and event checks.

Classes:
    TCalendar: Main calendar and campaign scheduling manager.
...
class TCalendar:
    """
    Manages the in-game calendar, campaign scheduling, and event checks.
    Advances time, triggers campaign and event logic, and provides date utilities.
    ...
    def _advance_one_day(self, *args, **kwargs):
        ...
        # Weekly, monthly, quarterly, yearly checks
    ...
    def generate_monthly_campaigns(self, month, research_status, all_campaigns, all_factions, worldmap):
        ...
        for faction_name in selected_factions:
            ...
            if not faction:
                continue
            # Check tech requirements for faction
            ...
            for camp in all_campaigns:
                if camp.faction != faction_name:
                    continue
                # Check tech requirements for campaign
            ...
            for mission in campaign.missions:
                # Check tech requirements for mission
        ...
        for camp in active_campaigns:
            for mission_entry in camp['missions']:
                if mission_entry['scheduled_day'] == current_day:
                    ...
                    # Create location for mission (placeholder)
                    camp['active_missions'].append(mission_entry)
                    # Optionally, remove from missions if one-time
                    if mission_entry['mission'].one_time:
                        camp['missions'].remove(mission_entry)
